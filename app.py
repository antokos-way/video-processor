...
def download_video():
    ...
    try:
        ...
        # Минимальные параметры с cookies
        base_params = [
        ...
        # Отладка: показываем все доступные форматы
        ...
        # Скачиваем ВИДЕО с поддержкой 60fps до 1440p (2K)
        video_filename = f"{folder}/video.%(ext)s"
        video_cmd = [
            'yt-dlp', video_url, 
            '-o', video_filename,
            '-f', 'bestvideo[height<=1440][fps<=60]+bestaudio/bestvideo[height<=1080][fps<=60]+bestaudio/bestvideo[height<=720][fps<=60]+bestaudio/best'
        ] + base_params
        ...
        if not video_files:
            return jsonify({
                ...
                'all_files': all_files,
                'debug_formats': debug_result.stdout[:500] if debug_result.returncode == 0 else 'Format check failed'
            }), 400
        ...
        return jsonify({
            ...
            'folder': folder,
            'debug_formats': debug_result.stdout[:500] if debug_result.returncode == 0 else None
        })
